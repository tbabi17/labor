		...
		if (records.length > 0) {
			Ext.Msg.confirm(Ext.sfa.translate_arrays[langid][540], Ext.sfa.translate_arrays[langid][541], function(btn, text){	                		
				if (btn == 'yes'){	
					...
					for (i = 0; i < records.length; i++) {
						var rec = records[i];								            		
						var values = me.customerCode+','+me.ticketID+','+me.users.getValue()+','+rec.data['productCode']+','+me.start.getText()+','+rec.data['quantity']+','+rec.data['driver']+','+rec.data['price']+','+rec.data['wareHouseID'];
