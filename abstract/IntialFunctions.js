...
generateFunction = function (tableName, modelName, funName, extend, fields, types, where) {
	...
	store.load({params:{xml:_donate(modelName, 'SELECT', tableName, fields, types, where)},
				callback: function() {							
...
loadOptimalDatas = function() {
	...
	comboData.load({params:{xml:_donate('Combos', 'SELECT', 'Combos', 'name,combo1,combo2,combo3,combo4', 's,s,s,s,s', ' ')}});
	
	settingsData.load({params:{xml:_donate('Settings', 'SELECT', 'Settings', 'name,value,descr', 's,s,s', " WHERE type=1 and (userCode='15' or userCode='"+module+"') ORDER by userCode")}, callback: function() {			
...
initStaticModels = function() {	
	...
	model['columns'] = [new Ext.grid.RowNumberer({width:30}),                 	                   
	...
	fields[1] = {name: 'first', type: 'float'};
	...
	columns = [];
	columns[0] = {name: 'discount', title: 'Харилцагч', width:130, renderer: Ext.sfa.renderer_arrays['renderParentID'], summaryType: 'sum', summaryRenderer: Ext.sfa.renderer_arrays['renderTMoney']};	
	columns[1] = {name: 'first', title: 'Эхний үлд', width:110, align: 'right', renderer: Ext.sfa.renderer_arrays['renderMoney'], summaryType: 'sum', summaryRenderer: Ext.sfa.renderer_arrays['renderTMoney']};
	
	
	var cls = [], t = 0;
	for (i = 1; i <= days[month]; i++) {
		var z = 'z'+i;
		cls[t] = {name: z, width: 90, align: 'right', title: month+'/'+i, summaryType: 'sum', renderer: Ext.sfa.renderer_arrays['renderMoney'], summaryRenderer: Ext.sfa.renderer_arrays['renderTMoney'],summaryType: 'sum', summaryRenderer: Ext.sfa.renderer_arrays['renderTMoney']};
		t++;
	}
	
	columns[2] = 
	{
		text: 'Зээлээр олгосон бараа',
		columns : cls
	};
	
	columns[3] = {name: 'ztotal', text: 'Нийт', width:100, align: 'right', renderer: Ext.sfa.renderer_arrays['renderMoney'], summaryType: 'sum', summaryRenderer: Ext.sfa.renderer_arrays['renderTMoney']};
	
	cls = [];
	var q = 0;
	for (i = 1; i <= days[month]; i++) {
		var t = 't'+i;
		cls[q] = {name: t, width: 90, align: 'right', title: month+'/'+i, summaryType: 'sum', renderer: Ext.sfa.renderer_arrays['renderMoney'], summaryRenderer: Ext.sfa.renderer_arrays['renderTMoney'], summaryType: 'sum', summaryRenderer: Ext.sfa.renderer_arrays['renderTMoney']};
		q++;
	}
	
	columns[4] = 
	{
		text: 'Төлбөр тооцоо авсан',
		columns : cls
	};
	
	columns[5] = {name: 'ttotal', title: 'Нийт', width:100, align: 'right', renderer: Ext.sfa.renderer_arrays['renderMoney'], summaryType: 'sum', summaryRenderer: Ext.sfa.renderer_arrays['renderTMoney']};
	columns[6] = {name: 'last', title: 'Эцсийн үлд', width:110, align: 'right', renderer: Ext.sfa.renderer_arrays['renderMoney'], summaryType: 'sum', summaryRenderer: Ext.sfa.renderer_arrays['renderTMoney']};
	...
	fields[0] = {name: 'discount', type: 'int'};
	fields[1] = {name: 'first', type: 'float'};	
	fields[2] = {name: 'ztotal', type: 'float'};
	fields[3] = {name: 'ttotal', type: 'float'};    	
	fields[4] = {name: 'last', type: 'float'};
	...
	columns = [];
	columns[0] = {name: 'discount', title: 'Харилцагч', width:150, renderer: Ext.sfa.renderer_arrays['renderParentID']};	
	columns[1] = {name: 'first', title: 'Эхний үлдэгдэл', width:130, align: 'right', summaryType: 'sum', renderer: Ext.sfa.renderer_arrays['renderMoney'], summaryRenderer: Ext.sfa.renderer_arrays['renderTMoney']};
	columns[2] = {name: 'ztotal', title: 'Зээлээр олгосон бараа', width:130, align: 'right', summaryType: 'sum', renderer: Ext.sfa.renderer_arrays['renderMoney'], summaryRenderer: Ext.sfa.renderer_arrays['renderTMoney']};
	columns[3] = {name: 'ttotal', title: 'Төлбөр тооцоо авсан', width:130, align: 'right', summaryType: 'sum', renderer: Ext.sfa.renderer_arrays['renderMoney'], summaryRenderer: Ext.sfa.renderer_arrays['renderTMoney']};
	columns[4] = {name: 'last', title: 'Эцсийн үлдэгдэл', width:130, align: 'right', summaryType: 'sum', renderer: Ext.sfa.renderer_arrays['renderMoney'], summaryRenderer: Ext.sfa.renderer_arrays['renderTMoney']};
